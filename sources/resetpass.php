<?php if(!defined('_source')) die("Error");  
   ...
   include_once _lib."class.database.php";
	...
	if($kiemtra['success']==''){
			...
			$sqluser= "select token,user,email from table_forgetpass where token='".$tokens."' order by id desc";
			$d->query($sqluser);		
			$kquser = $d->result_array();
										...
										<td>
											...
											<div style="text-align:left;padding:0px 20px 20px">
												<h1 class="m_-6358922000748629568header" style="font-size:1.2em">
													<span style="color:#202020">Hi </span><b style="color:#3498db">'.$kquser[0]['user'].',</b>
												</h1><br>
												<p style="color:#f00">
			...
			$mail->AddAddress($kquser[0]['email'],'BitBankCoin');
			
			//Thiết lập email nhận email hồi đáp
			//nếu người nhận nhấn nút Reply
			$mail->AddReplyTo($kquser[0]['email'],'BitBankCoin');
