...
function get_items(){
	...
	#----------------------------------------------------------------------------------------
		if($_REQUEST['hienthi']!=''){
			...
			$sql_sp = "SELECT id,maso,hienthi FROM table_product where id='".$id_up."' ";
			$d->query($sql_sp);
			$cats= $d->result_array();
			...
			if($hienthi==0){
				...
				$sqlUPDATE_ORDER = "UPDATE table_product SET hienthi =1,ngaykichhoat=".strtotime(date("Y-m-d",time())).",nguoikichhoat='".$_SESSION['login']['username']."' WHERE  id = ".$id_up."";
	...
	$sql = "select * from #_product where maso !='' ";
	...
	if($_REQUEST['trangthai']!=0){
		$luutam=$_REQUEST['trangthai']-1;
	    $sql.=" and hienthi=".$luutam."";
		$bien="&trangthai=".$luutam;	
	}
	 ...
	 if($_REQUEST['keyword']!='')
	{
		...
		$bien1="&keyword=".$keyword;	
	}
	
	
	if($_REQUEST['batdau'] != "" ) {$bien2 ="&batdau=".$_REQUEST['batdau'];}
	if($_REQUEST['ketthuc'] != "" ) {$bien3 ="&ketthuc=".$_REQUEST['ketthuc'];}
	...
	$url ="index.php?com=product&act=man".$bien.$bien1.$bien2.$bien3.$bien4;		
	$maxR=30;
	$maxP=20;
...
function save_item(){
	...
	if($_POST['id_cat1s'] != "" ) {$bien1=$_POST['id_cat1s'];}
	if($_POST['trangthais'] != "" ) {$bien2 =$_POST['trangthais'];}
	if($_POST['batdaus'] != "" ) {$bien3 =$_POST['batdaus'];}
	if($_POST['ketthucs'] != "" ) {$bien4 =$_POST['ketthucs'];}
	if($_POST['curPage'] != "" ) {$bien5 =$_POST['curPage'];}
	...
	if($id){
		...
		$data['tenkhongdau'] = changeTitle($_POST['hoten']);		
		$data['email'] = $_POST['email'];					
		if($_POST['matkhau']!="")
			$data['matkhau'] = taomatkhau($_POST['matkhau']);
		$data['diachi'] = $_POST['diachi']; 
		$data['dienthoai'] = $_POST['dienthoai'];
		$data['vi'] = $_POST['dogecoin'];
		...
		if($d->update($data))
			...
			redirect("index.php?com=product&act=man".$bien.$bien1.$bien2.$bien3.$bien4.$bien5);
		else
		...
		if($_POST['sopin']!=''){
			...
			$dataq['masocho'] = 'ADMIN';
			$dataq['sopin'] = $_POST['sopin'];
			$dataq['masonhan'] = $maso;	
			$dataq['ngaycho'] = strtotime(date("Y-m-d H:i:s",time()));		
			$d->setTable('chuyenpin');
			$d->insert($dataq);		
		}
		
		
		$gt=explode("-",$_POST['gioithieu']);
		...
		$data['bitcoin'] = $_POST['bitcoin'];
		$data['diachi'] = $_POST['diachi'];
		$data['vi'] = $_POST['dogecoin'];  
		$data['sopin'] = $_POST['sopin'];
		$data['hienthi'] = 1;
		$data['ngaytao'] = strtotime(date("Y-m-d H:i:s",time()));		
		$data['nguoitao'] = 'Admin';
		$data['ngaykichhoat'] = strtotime(date("Y-m-d H:i:s",time()));		
		$data['nguoikichhoat'] = 'Admin';							
		$d->setTable('product');
		$d->insert($data);
		
			
		transfer("Thêm mới thành công", "index.php?com=product&act=man");
